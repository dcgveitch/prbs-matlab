...
for d_impulse=d_reqImp
    for d_conc=d_reqConc
        outB_graphR=cell(1,3);
        outB_graphP=cell(1,3);
        for d_flowType=1:3
            ...
            for d_perm=1:setup_nSim
                ...
                for d_flowCount=1:d_nFlows
                    ...
                    outB_graphP{d_solve,d_flowType}(d_count1,9)=wmean(d_flowOutput{d_flowCount}(:,1),d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,8)=outB_graphP{d_solve,d_flowType}(d_count1,9)-wstd(d_flowOutput{d_flowCount}(:,1),d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,10)=outB_graphP{d_solve,d_flowType}(d_count1,9)+wstd(d_flowOutput{d_flowCount}(:,1),d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,11)=wprctile(d_flowOutput{d_flowCount}(:,1),15.87,d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,12)=wprctile(d_flowOutput{d_flowCount}(:,1),25,d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,13)=wprctile(d_flowOutput{d_flowCount}(:,1),50,d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,14)=wprctile(d_flowOutput{d_flowCount}(:,1),75,d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,15)=wprctile(d_flowOutput{d_flowCount}(:,1),84.13,d_flowOutput{d_flowCount}(:,3));
                    outB_graphP{d_solve,d_flowType}(d_count1,16)=sum(d_flowInput{d_flowCount}(:,2));
                    d_count1=d_count1+1;
                end
            end          
        end
