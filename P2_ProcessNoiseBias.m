    ...
    for ref_bPerm = 1:d_batchSize
        ...
        for d_conc=d_reqConc
            ...
            for d_noise=1:size(sim_conc{d_conc},3)
                if (mod(d_noise,10)==1), disp(['  --Run ' num2str(ref_bPerm) ' Conc ' num2str(d_conc) ' Noise ' num2str(d_noise) '/' num2str(size(sim_conc{d_conc},3))]); end
                %%%
                if (clc_afType=='S' || clc_afType=='F')
                    ...
                    if (ismember(1,d_reqImp) || ismember(2,d_reqImp))
                        for d_relZone=1:clc_nZones
                            for d_concZone=1:clc_nZones
                                ...
                                if d_conc==4
                                    d_crossCorr=outB_prbsCrossCorr{ref_bPerm}{1}{1}(:,d_relZone,d_concZone,d_seqV);
                                    d_crossCorrMax=max(d_crossCorr);
                                    d_crossCorr=d_crossCorr+randn(length(d_crossCorr),1)*d_crossCorrMax*(ref_bPerm*0.2/10);
                                end                                
...
%                 end
    ...
    mat_outP2.out_fit(1,d_batchRun)=outB_fit;
...
d_procTime=toc
mat_outP2.d_procTime=d_procTime;
